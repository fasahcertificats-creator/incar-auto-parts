...
import { AdminApiError, adminGetCategory, adminUpdateCategory } from "@/features/admin/api/client";
import {
...
type DetailState =
  ...
  | { kind: "ready"; category: AdminCategory };
...
export default function AdminCategoryDetailPage() {
  ...
  useEffect(() => {
    let cancelled = false;
    adminGetCategory(id)
      .then((category) => !cancelled && setState({ kind: "ready", category }))
      .catch((caught: unknown) => {
        ...
        setState({
          kind: "error",
          message: caught instanceof AdminApiError ? caught.message : "Failed to load category.",
        });
      });
    return () => {
      cancelled = true;
  ...
  async function handleSave(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setState({ kind: "ready", category: updated });
    } catch (caught) {
  ...
  if (state.kind === "not-found") return <p className="text-sm text-muted">Category not found.</p>;
  if (state.kind === "error") {
    return (
  ...
  return (
    <div>
      <Link href="/admin/categories" className="incar-focus text-sm text-metallic-silver hover:text-white">
        ← Back to categories
      </Link>
      <h1 className="mt-4 text-2xl font-semibold text-white">{category.nameEn}</h1>
      <p className="mt-1 text-sm text-muted">{category.productCount} linked products.</p>

      <form onSubmit={handleSave} className="incar-card mt-6 grid gap-4 rounded-lg p-6 sm:max-w-2xl sm:grid-cols-2">
        <label className="grid gap-2 text-sm font-semibold text-white">
          Slug
          <input name="slug" defaultValue={category.slug} required className="incar-input px-4 text-sm" />
        </label>
        <label className="grid gap-2 text-sm font-semibold text-white">
          Status
          <select name="status" defaultValue={category.status} className="incar-input px-4 text-sm">
        ...
        <label className="grid gap-2 text-sm font-semibold text-white">
          Name (Arabic)
          <input name="nameAr" dir="rtl" defaultValue={category.nameAr} required className="incar-input px-4 text-sm" />
        </label>
        <label className="grid gap-2 text-sm font-semibold text-white">
          Name (English)
          <input name="nameEn" defaultValue={category.nameEn} required className="incar-input px-4 text-sm" />
        </label>
